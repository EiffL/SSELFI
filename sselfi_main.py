...
flags.DEFINE_integer(
    'num_parameters', default=2, help='Number of parameters to infer.')

flags.DEFINE_integer(
    'mixture_size', default=16, help='Number of components in Gaussian mixture')

flags.DEFINE_integer(
    'bottleneck_size', default=2, help='Size of bottleneck for summary'
                                       'statistic, at least 1.')
...
flags.DEFINE_integer(
    ...
    help=('The number of steps to use for training. Default is 50000 steps'
...
def resnet_model_fn(features, labels, mode, params):
  ...
  out_mu = tf.layers.dense(inputs=net,
                           units=FLAGS.num_parameters * FLAGS.mixture_size)
  out_mu = tf.reshape(out_mu, (-1, FLAGS.mixture_size, FLAGS.num_parameters))

  # Outputs mixture covariances
  size_sigma = (FLAGS.num_parameters * (FLAGS.num_parameters +1) // 2)
  out_sigma = tf.layers.dense(inputs=net,
                              units= size_sigma * FLAGS.mixture_size)
  out_sigma = tf.reshape(out_sigma, (-1, FLAGS.mixture_size, size_sigma))
  sigma_mat = tfd.matrix_diag_transform(tfd.fill_triangular(out_sigma),
                                        transform=tf.nn.softplus)

  # Outputs mixture weights
  out_p = tf.layes.dense(inputs=net, FLAGS.mixture_size, activation_fn=None)
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    predictions = {
        ...
        'p': out_p,
        'sigma': out_sigma,
        'samples': tf.transpose(gmm.sample(1024), [1, 2, 0])  # Moves the samples to the last dimension
  ...
  loglik = - tf.reduce_mean(gmm.log_prob(labels),axis=0)

  # Add weight decay to the loss for non-batch-normalization variables.
  loss = loglik + FLAGS.weight_decay * tf.add_n(
