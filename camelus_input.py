...
# ==============================================================================
"""Efficient Camelus input pipeline using tf.data.Dataset."""
...
import camelus_preprocessing

TRAINING_SHARD_SIZE = 20  # We keep 20480 examples for training exactly, the rest can be used for testing

def map_serving_input_fn():
  """Serving input fn for raw convergence maps."""

  def _preprocess_image(image_bytes):
    """Preprocess a single raw image."""
    image = camelus_preprocessing.preprocess_image(
...
class CamelusInput(object):
  """Generates Camelus input_fn for training.
  ...
  The format of the data required is created by the script at:
      camelus/utils/upload_to_gcs.py
  ...
  def __init__(self, is_training, data_dir, use_bfloat16, transpose_input=True):
    self.image_preprocessing_fn = camelus_preprocessing.preprocess_image
  ...
  def dataset_parser(self, value):
    ...
    keys_to_features = {
        ...
        'map/width':
            tf.FixedLenFeature([], tf.int64, -1),
        'map/param/omegam':
            tf.FixedLenFeature([], tf.float32, -1),
        'map/param/sigma8':
            tf.FixedLenFeature([], tf.float32, -1),
        'map/filename':
            tf.FixedLenFeature([], tf.string, ''),
    ...
    image_bytes = tf.reshape(parsed['map/buffer'], shape=[])

    # Preprocess and convert image to float
    image = self.image_preprocessing_fn(
        image_bytes=image_bytes,
        is_training=self.is_training,
    )

    label = tf.concat([tf.cast(tf.reshape(parsed['map/param/omegam'], shape=[1]), tf.float32),
                      tf.cast(tf.reshape(parsed['map/param/sigma8'], shape=[1]), tf.float32)],axis=-1)
  ...
  def input_fn(self, params):
    ...
    def fetch_dataset(filename):
      ...
      if is_training:
          dataset = dataset.take(TRAINING_SHARD_SIZE)
      else:
          dataset = dataset.skip(TRAINING_SHARD_SIZE)
      return dataset
  ...
  def _get_null_input(self, _):
    null_image = tf.zeros([224, 224, 1], tf.bfloat16
